...
public class UserService implements IUserService {
    ...
    public List<User> getAllUserScoreOrderWise(ScoreOrder scoreOrder) {
        ...
        if (scoreOrder == ScoreOrder.ASC)
            Collections.sort(users, (a, b) -> a.getScore() - b.getScore());
        else
            Collections.sort(users, (a, b) -> b.getScore() - a.getScore());
    ...
    public UserRegistrationDto withdrawContest(String contestId, String userName)
        ...
        if (!user.checkIfContestExists(contest)) {
            throw new InvalidOperationException(
                    "Cannot Withdraw Contest. User not registered for given contest id:"
                            + contestId);
        }
