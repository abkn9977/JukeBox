...
public class QuestionRepository implements IQuestionRepository {
    ...
    public List<Question> findAll() {
        return questionMap.entrySet().stream().map(e -> e.getValue()).collect(Collectors.toList());
    }
    ...
    public List<Question> findAllQuestionLevelWise(Level level) {
        ...
        return questionMap.entrySet().stream().map(e -> e.getValue())
                .filter(q -> q.getLevel() == level).collect(Collectors.toList());
    }
