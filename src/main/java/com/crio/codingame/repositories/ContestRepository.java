...
public class ContestRepository implements IContestRepository {
    ...
    public List<Contest> findAll() {
        return contestMap.entrySet().stream().map(e -> e.getValue()).collect(Collectors.toList());
    }
    ...
    public List<Contest> findAllContestLevelWise(Level level) {
        return contestMap.entrySet().stream().map(c -> c.getValue())
                .filter(l -> l.getLevel() == level).collect(Collectors.toList());
    }
}
